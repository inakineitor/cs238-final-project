...
def main():
    ...
    for map_type in  ["BG", "TRACT", "COUSUB", "COUNTY"]: # "BLOCK" also
        with open(f"do_not_commit/{map_type}_face_stats.json") as f:
            data = json.load(f) # {state name: [number of total faces, [faces with 1 side, 2 sides, 3 sides, ...]]}
        ...
        plt.hist(range(1, max_display_face_size + 1), bins=max_display_face_size, weights=[face_stats[j] / sum(face_stats) for j in range(max_display_face_size)])
        plt.title("Face Sides")
        largest_face_size = max([j for j in range(500) if face_stats[j] > 0]) + 1
        plt.text(0.5, -0.1, f"Percentage of faces not depicted: {round(sum(face_stats[max_display_face_size:]) / sum(face_stats), 4) * 100}%; Largest face number of sides: {largest_face_size}", transform=plt.gca().transAxes, ha='center')
