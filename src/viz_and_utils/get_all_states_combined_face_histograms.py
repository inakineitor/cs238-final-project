...
def plot_and_save_and_show(title, file_path):
    ...
    fig_standard, axs_standard = plt.subplots(
        (len(data) + 4) // 5, 5, figsize=(9, 2.2 * ((len(data) + 4) // 5))
    )
    axs_standard = axs_standard.flatten()

    # Prepare figure for plotting histograms with log scale
    fig_log, axs_log = plt.subplots(
        (len(data) + 4) // 5, 5, figsize=(9, 2.2 * ((len(data) + 4) // 5))
    )
    ...
    for index, (state, (length, array_A)) in enumerate(data.items()):
        ...
        axs_standard[index].hist(
            range(15), weights=first_15_elements, bins=15, alpha=0.75
        )
        ...
        # Plot with log scale
        axs_log[index].hist(
            range(15), weights=first_15_elements, bins=15, alpha=0.75, log=True
        )
        ...
        non_zero_beyond_15 = [
            (i, val) for i, val in enumerate(array_A[15:], start=15) if val != 0
        ]
        if non_zero_beyond_15:
            print(
                f"State {state} has non-zero values beyond index 15: {non_zero_beyond_15}"
            )
