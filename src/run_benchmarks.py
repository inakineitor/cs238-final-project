...
from map_analyzer.benchmark_framework.benchmark_orchestrator import (
    BenchmarkOrchestrator, BenchmarkOrchestratorResults
)
from map_analyzer.benchmarks import maximum_degree, radius, number_of_leaves, diameter, assortativity, average_degree, clustering_coefficient, face_if_planar
...
def main():
    ...
    num_vertices, graph_nodes = orchestrator.benchmark_model(models_to_test[0], NUM_ITERS, SEED)
    ...
    for i in range(1, len(models_to_test)): 
        if i == 1: # triangle deletion
            ...
            params['model'] = 'tri'
            nvert, graph_nodes = orchestrator.benchmark_model(models_to_test[i], NUM_ITERS, SEED, extra_params=params)
